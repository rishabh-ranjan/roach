# ILC: what a batch job's environment looks like. Sourced, on every node a job
# holds, before the clone is touched.
#
# A batch script is not a login shell, so none of the interactive config runs.
# This goes through the same entry point an interactive login does --
# .bashrc.user in the real (passwd) home -- which runs setup-node.sh: a
# node-local home that is a dotfiles checkout, with a node-local pixi and the
# global CLI tools. Without --update, so it never pulls a node out from under
# a running job.
#
...
# Sourcing .bashrc.user non-interactively stops before the fish exec. It also
# cd's to the new HOME, hence the restore.
...
# a few hundred GB shared with the OS -- and a job whose scratch fills it wedges
# itself and every other job on the node. /lfs/local/0 is a multi-terabyte NVMe
# mounted per node for exactly this, and a resume point kept there survives a
# requeue.
...
# number here could only disagree with the allocation.
